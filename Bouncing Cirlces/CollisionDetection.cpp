...
{/*
	...
	*/
	sf::Vector2f vectorOffset(obj1.getPosition().x - shape2.getPosition().x, obj1.getPosition().y - shape2.getPosition().y);

	for (int i = 0; i < shape1.getShape().getPointCount(); i++)
	{
		sf::Vector2f axis = getNormalAxis(shape1.getVertexArray(), i);


		float min1 = dotProduct(axis, shape1.getVertexArray()[0].position);
		//float min1 = dotProduct(axis, shape1.getShape()[0].position);
		float max1 = min1;

		for (int j = 1; j < shape1.getShape().getPointCount(); j++)
		{
			//float testNum = dotProduct(axis, shape1.getShape()[j].position);
			float testNum = dotProduct(axis, shape1.getVertexArray()[j].position);
			if (testNum < min1)
				min1 = testNum;
			if (testNum > max1)
				max1 = testNum;
		...
		float min2 = dotProduct(axis, shape2.getVertexArray()[0].position);
		float max2 = min2;

		for (int j = 1; j < shape2.getShape().getPointCount(); j++)
		{
			//float testNum = dotProduct(axis, shape2.getShape()[j].position);
			float testNum = dotProduct(axis, shape2.getVertexArray()[j].position);
			if (testNum < min2)
				min2 = testNum;
			if (testNum > max2)
				max2 = testNum;
...
std::vector<sf::Vector2f> CollisionDectection::getAxis(vector<sf::Vector2f> pointArray)
{
	...
	for (int i = 0; i < pointArray.size(); i++)
	{
		...
		edge = sf::Vector2f(-edge.y, edge.x);	//normalize the vector
